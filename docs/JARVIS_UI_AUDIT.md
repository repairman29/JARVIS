...
| 2.7 | Structured tool output (lists/tables/expandable) | ✅ Done | Message.tsx: `StructuredResultView` renders `meta.structured_result` (list, table, key_value, or expandable JSON). API + Chat pass-through; contract in [JARVIS_UI_GATEWAY_CONTRACT.md](./JARVIS_UI_GATEWAY_CONTRACT.md). |
...
**Phase 2:** All done. 2.6 ready when gateway sends meta.tools_used; 2.7 StructuredResultView when gateway sends meta.structured_result.
...
| 4.8 | CLI parity ("run and copy result") | ✅ Done | Header "Run and copy result" button; Composer ref getValue(); non-stream request, copy to clipboard. |

**Phase 4:** All done (4.8 Run and copy result implemented).

---

## Summary: What to Do Next

1. **Phase 2:** Tool visibility (2.6) — UI ready; gateway/Edge send `meta.tools_used`. Structured output (2.7) — ✅ UI renders list/table/key_value/expandable JSON when gateway sends `meta.structured_result`.
2. **Phase 4:** CLI parity (4.8) — Done: "Run and copy result" button uses non-stream request and copies response.
