#!/usr/bin/env bash
# Refresh the Pixel IP used by jarvis-chat, ssh-pixel, etc. (e.g. after switching WiFi: work ↔ home).
# Usage:
#   ./scripts/pixel-refresh-ip.sh              # discover via ADB, update .pixel-ip
#   ./scripts/pixel-refresh-ip.sh 192.168.1.50 # set IP explicitly
#
# All scripts that use .pixel-ip or JARVIS_PIXEL_IP will then use the new IP.
...
echo ""
echo "On new WiFi, either:"
echo "  1. Connect Pixel via USB and run:  ./scripts/pixel-refresh-ip.sh"
echo "  2. On the Pixel (Termux): run  ifconfig  and note the inet under wlan0"
echo "  3. Then on Mac:  ./scripts/pixel-refresh-ip.sh <that-ip>"
