...
- **Edge secrets:** Set `JARVIS_GATEWAY_URL`, `CLAWDBOT_GATEWAY_TOKEN`, optional `JARVIS_AUTH_TOKEN` in Dashboard → Edge Functions → jarvis → Secrets (or `supabase secrets set`). **Edge vs Pixel farm:** See [docs/JARVIS_EDGE_AND_PIXEL_FARM.md](docs/JARVIS_EDGE_AND_PIXEL_FARM.md) — Edge and Pixel/iPhone farm connect only when `JARVIS_GATEWAY_URL` points at the Pixel (e.g. via tunnel).
